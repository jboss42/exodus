...
if ($::RTYPE eq "daily") {
  ...
  e("$::SSH $userhost \"cd $::ROOT/www/daily/stage; chmod 664 *; mv setup.exe ..; mv Exodus.zip ..; mv *.zip ../plugins\; $::GENLOG $oldrev daily\"");
} else {
  ### RELEASE BUILDS
  my $uver;
  ...
  e("$::SSH $userhost \"chmod 664 $::ROOT/files/exodus_$version.exe $::ROOT/www/plugins/*.zip ; $::GENLOG $oldrev release\"");
}
